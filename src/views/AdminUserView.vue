<template>
    <v-container fluid class="pa-8">
          ...
          <v-menu offset-y>
            <template v-slot:activator="{ on, attrs }">
              <v-btn class="primary" large rounded color="primary" dark v-bind="attrs" v-on="on">
                <v-icon>mdi-account-plus</v-icon><div class="pl-1">Create Admin</div>
              </v-btn>
            </template>
            <v-list>
              <v-list-item @click="() => { formbtnBool = false; dialog = true; }">
                <v-list-item-title>
                  <v-icon>mdi-account-check</v-icon>CREATE NEW ADMIN
                </v-list-item-title>
              </v-list-item>
            </v-list>
          </v-menu>
            ...
            <v-container fluid class="pa-0">
              <v-card-text class="text-center">
                ...
                <div class="d-flex justify-space-between pt-4 pb-2" fluid>
                  <v-btn depressed class="secondary black--text" large width="157px" rounded @click="
                    () => {
                      deleteDialog = false;
                      formbtnBool = false;
                    }
                  ">CANCEL</v-btn>
                  <v-btn class="black white--text" depressed large width="157px" rounded :disabled="selected.length == 0"
                    @click="deleteUser(selected)">DELETE</v-btn>
                </div>
              </v-card-text>
            </v-container>
          </v-card>
      ...
      <v-data-table v-model="selected" :headers="headers" :items="users" show-select :single-select="singleSelect"
        item-key="id" :options.sync="options"
        :footer-props="{
        itemsPerPageOptions: [10, 25, 50, 100]
        }"  :server-items-length="count" class="usertable">
            ...
            <v-toolbar elevation="0" class="secondary">
              <v-row>
                <v-col cols="12" sm="12" md="12">
                  <v-toolbar-title class="text-h5"> {{ formbtn() }} Admin</v-toolbar-title>
                </v-col>
              ...
              <v-stepper v-model="e1">
                ...
                <v-stepper-items>
                  ...
                  <v-stepper-content step="1">
                          ...
                          <v-col cols="12">
                            <v-card class="pa-4" elevation="0">
                              <v-row class="py-0">
                                ...
                                <v-col cols="4" class="py-0"><v-text-field v-model="personalInfo.first_name"
                                    :rules="[v => !!v || 'First Name is required']" required outlined label="First Name *"
                                    rounded class="rounded-xl"></v-text-field></v-col><v-col cols="3"
                                  class="py-0"><v-text-field v-model="personalInfo.middle_name" outlined label="Middle Name"
                                    rounded class="rounded-xl"></v-text-field></v-col><v-col cols="3"
                                  class="py-0"><v-text-field v-model="personalInfo.last_name" outlined label="Last Name"
                                    rounded class="rounded-xl"
                                    :rules="[v => !!v || 'Last Name is required']"></v-text-field></v-col>
                              </v-row>
                              <v-row class="py-0">
                                <v-col class="py-0"><v-text-field v-model="personalInfo.email" outlined
                                    label="Email Address *" rounded class="rounded-xl" required
                                    :rules="[v => !!v || 'Email Address is required']"></v-text-field></v-col>
                              </v-row>
                              <v-row class="py-0">
                                <v-col class="py-0"><v-text-field label="Your 10-digit mobile no. *" outlined rounded
                                    class="rounded-xl" counter="10" prefix="+91"
                                    :rules="[v => !!v || 'Mobile Number is required']" required maxLength="10"
                                    v-model="personalInfo.phone_no"></v-text-field></v-col>
                              </v-row>
                              <v-row class="py-0">
                                <v-col cols="4" class="py-0">
                                  <v-text-field outlined label="Date of Birth (DDMMYY) *" rounded class="rounded-xl"
                                    type="date" :rules="[v => !!v || 'Date of Birth is required']"
                                    v-model="personalInfo.dob"></v-text-field></v-col>
                                <v-col cols="2" class="py-0">
                                  <v-select label="Gender *" :items="['MALE', 'FEMALE','OTHERS']" outlined class="rounded-xl"
                                    v-model="personalInfo.gender" :rules="[v => !!v || 'Gender is required']"  required>
                                  </v-select>
                                </v-col>
                              </v-row>
                              ...
                              <v-row class="py-0">
                                ...
                                <v-col cols="6" class="py-0">
                                  <v-select v-model="personalInfo.state_id" label="State" :items="states" outlined
                                    class="rounded-xl" item-value="id" item-text="state_name" @change="fetchDistricts" :rules="[v => !!v || 'State is required']" required>
                                  </v-select>
                                </v-col>
                              </v-row>
                              <v-row class="py-0">
                                <v-col cols="6" class="py-0">
                                  <v-select v-model="personalInfo.districtId" label="District" :items="districts" outlined
                                    class="rounded-xl" item-value="id" item-text="district_name" @change="fetchTalukas" :rules="[v => !!v || 'District is required']" required>
                                  </v-select>
                                </v-col>
                                <v-col cols="6" class="py-0">
                                  <v-select v-model="personalInfo.talukTehsil" label="Taluk / Tehsil" :items="talukas"
                                    outlined class="rounded-xl" item-value="id" item-text="taluka_name"
                                    @change="fetchCities" :rules="[v => !!v || 'Tehsil is required']" required>
                                  </v-select>
                                </v-col> </v-row><v-row class="py-0">
                                <v-col cols="6" class="py-0">
                                  <v-select v-model="personalInfo.city_id" label="City / Village" :items="cities" outlined
                                    class="rounded-xl" item-value="id" item-text="city_name" :rules="[v => !!v || 'City is required']" required>
                                  </v-select>
                                </v-col>
                                <v-col cols="6" class="py-0">
                                  <v-text-field v-model="personalInfo.pincode" outlined label="Pin Code" rounded maxLength="6" counter="6"
                                    class="rounded-xl" :rules="[v => !!v || 'Pincode is required']" required></v-text-field>
                                </v-col>
                    ...
                    <v-btn rounded color="primary" class="mx-4 my-4" @click="goToStep2">
                      NEXT
                    </v-btn>
                  </v-stepper-content>
                  <!------------------------------------------ STEP 2 ------------------------------------------>
                        ...
                        <v-card class="pa-4" elevation="0">
                          <v-expansion-panels v-model="expandedPanelIndex">
                            <v-expansion-panel v-for="(
                                      qualification, index
                                    ) in academicQualifications" :key="index" elevation="0">
                              ...
                              <v-expansion-panel-content>
                                <v-row class="py-0">
                                  <v-col class="py-0"><v-text-field v-model="qualification.institution" outlined
                                      label="School/ College/ University *" rounded class="rounded-xl"
                                ...
                                <v-row class="py-0">
                                  <v-col cols="6" class="py-0"><v-text-field v-model="qualification.start_date" outlined
                                      ...
                                      :rules="[v => !!v || 'Start Date is required']"></v-text-field></v-col>
                                  <v-col cols="6" class="py-0"><v-text-field v-model="qualification.end_date" outlined
                                      ...
                                      :rules="[v => !!v || 'End Date is required']"></v-text-field></v-col>
                                </v-row>
  
                                <v-row class="py-0">
                                  <v-col cols="12" class="py-0">
                                    <v-text-field v-model="extra_carricular_activities" outlined
                                ...
                                <v-row>
                                  <v-col cols="12" class="d-flex justify-space-between">
                                    ...
                                    <v-btn v-if="academicQualifications.length != 1" @click="openDeleteDiolog(index)" text
                  ...
                  <v-stepper-content step="3">
                    <v-form lazy-validation ref="step3">
                      <v-card elevation="0">
                        <v-card class="pa-4" elevation="0">
                          <v-expansion-panels v-model="expandedPanelIndex">
                            <v-expansion-panel v-for="(professional, index) in professionalInfos" :key="index"
                              elevation="0">
                              <v-expansion-panel-header>
                                <div class="d-flex flex-column" v-if="expandedPanelIndex != index">
                                  <div class="font-weight-regular">
                              ...
                              <v-expansion-panel-content>
                                <v-row class="py-0">
                                  <v-col class="py-0">
                                    <v-card width="100%" elevation="0" class="mb-10 rounded-xl" outlined><v-radio-group
                                        mandatory row v-model="experience" :rules="[v => !!v || 'Please select one']"
                                        required>
                                        <v-col class="py-0">
                                          <v-row class="py-0 px-0 ml-2">
                                            ...
                                            <v-col cols="1 center" class="py-0 px-0">
                                              <v-text-field type="number" outlined @keypress="isNumber($event)"
                                                class="rounded-xl" v-model="
                                            ...
                                            <v-col cols="1" class="py-0 px-0">
                                              <v-text-field type="number" @keypress="isNumber($event)" outlined
                                                class="rounded-xl" v-model="
                                ...
                                <div v-if="experience == 'Experienced'"> <v-row class="py-0">
                                    <v-col class="py-0"><v-text-field outlined label="Role/ Position *" rounded
                                        class="rounded-xl" counter="100" maxLength="100"
                                        :rules="[v => !!v || 'Role/ Position is required']" required
                                  ...
                                  <v-row class="py-0">
                                    <v-col class="py-0"><v-select label="School / Institute" :items="schoolData"
                                        item-value="id" item-text="name" outlined class="rounded-xl">
                                      </v-select></v-col>
                                  </v-row>
                                  <v-row class="py-0">
                                    <v-col class="py-0">
                                  ...
                                  <v-row class="py-0">
                                    ...
                                    <v-col cols="6" class="py-0"><v-text-field :disabled="isCurrentlyWorking" outlined
                                        label="End Date" rounded class="rounded-xl" v-model="professional.end_date"
                                        type="date" :rules="[v => !!v || 'End Date is required']"></v-text-field></v-col>
                                  </v-row>
                                  <v-row class="py-0">
                                    <v-col class="py-0"><v-select label="Board" :items="boardsData" item-text="name"
                                        item-value="id" outlined class="rounded-xl" v-model="professional.board_id">
                                      </v-select></v-col> </v-row><v-row class="py-0">
                                    <v-col class="py-0"> <v-autocomplete clearable deletable-chips label="Levels" outlined
                                    ...
                                    <v-col class="py-0"><v-autocomplete clearable deletable-chips label="Grades" outlined
                                        class="rounded-xl" small-chips :items="gradesData" multiple item-text="name"
                                        item-value="id">
                                      </v-autocomplete></v-col> </v-row><v-row class="py-0">
                                    <v-col class="py-0"><v-autocomplete clearable deletable-chips label="Subjects" outlined
                                  ...
                                  <v-row class="py-0">
                                    <v-col class="py-0">
                                      <v-autocomplete clearable deletable-chips label="Which board do you want to teach?"
                                        outlined class="rounded-xl" small-chips :items="boardsData" item-text="name"
                                        item-value="id" multiple>
                                      </v-autocomplete>
                                    </v-col> </v-row><v-row class="py-0">
                                    <v-col class="py-0">
                                      <v-autocomplete clearable deletable-chips label="Which level do you want to teach?"
                                        outlined class="rounded-xl" small-chips :items="tableLevels" multiple item-text="name"
                                        item-value="id">
                                      </v-autocomplete>
                                    </v-col>
                                  </v-row>
                                  ...
                                  <v-row class="py-0">
                                    <v-col class="py-0">
                                      <v-autocomplete clearable deletable-chips label="Which subject do you want to teach?"
                                        outlined class="rounded-xl" small-chips :items="subjectsData" multiple item-text="name"
                                        item-value="id">
                                      </v-autocomplete>
                                    </v-col>
                                  </v-row>
                                ...
                                <v-row >
                                  <v-col cols="12" class="d-flex justify-space-between">
                                    <v-btn text rounded outlined v-if="formbtnBool"
                                      @click="updateProfessionalInfo(professional, editId)">Update</v-btn>
  
                                    <v-btn v-if="experience == 'Experienced' && professionalInfos.length != 1" @click="openDeleteDiolog(index)" text
                                      class="d-flex justify-end red--text">Remove</v-btn>
                                  </v-col>
                                </v-row>
                              </v-expansion-panel-content>
                    ...
                    <v-container>
                      <v-row>
                        ...
                        <v-btn :loading="isCreatingUser" rounded depressed color="primary" class="my-4" @click="createUser">
                          {{ formbtn() }} DETAILS
                        </v-btn>
                      </v-row>
                    </v-container>
        ...
        <v-card>
          <v-container fluid class="pa-8">
            <v-card-text class="text-center">
              ...
              <p class="text-h5 py-4">New User {{ formbtnBool == false ? 'Created' : 'Updated' }}</p>
        ...
        <v-card>
          <v-container fluid class="pa-8">
            <v-card-text class="text-center">
              ...
              <v-btn class="primary" large width="157px" rounded @click="resendInviteSuccessDialog = false">OK</v-btn>
            </v-card-text>
          </v-container>
        </v-card>
      </v-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      updateData(item) {
        ...
        this.personalInfo.middle_name = item.middle_name,
        ...
        this.personalInfo.pincode = item.pincode,
          //   this.professionalInfos.experience_year = item.experienceYear,
          //   this.professionalInfos.experience_month = item.experienceMonth,
          //   this.professionalInfos.position = item.position,
          //   this.professionalInfos.employee_type_id = item.employee_type_id,
          //   this.professionalInfos.board_id = item.board_id,
        this.dialog = true,
