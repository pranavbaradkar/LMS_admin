<template>
  <v-container fluid class="pa-8">
    <v-row>
      <v-col>
        <v-btn @click="dialog = true ,newCreateBrands()" class="primary" large rounded><v-icon>mdi-plus</v-icon>Create Brand</v-btn>
        <v-dialog max-width="887px" v-model="dialog" center>
          <v-form ref="form" lazy-validation>
            <v-card>
              <v-card-title class="secondary mb-8"> {{ formbtn() }} Brand</v-card-title>
              <v-card-text class="px-6 pb-0">
                <v-text-field outlined class="rounded-xl" v-model="name" label="Enter Brand*" :rules="[v => !!v || 'Brand name is required']"
